...
import warnings
from matplotlib import pyplot as plt
from Parameter_file_Singularity_time_structure import potential,dxpotential,dypotential,ddpotential1,ddpotential2,ddpotential3,ddpotential4
...
def set_target(xcoord):
    global xtarget
    xtarget = xcoord
    
singtime_arr = []

def singtime_finder(xinit,yinit,pxinit,pyinit):    
    global singtime_arr,xtarget,dtim,xtarget,digress
    digress = 0 
    phasejumparr=[]
    dtim=0.0
    # xtarget=0.0j
    ...
    #print(digress,phasejumparr,dtim,xtarget)
    
    result = finalcondition(fq,1e3,xinit,yinit,pxinit,pyinit,0.0)
    
    singtime_arr.append(result[4])
    initpointxcoord = result[0]
    initpointtime = result[4]
    #print('first singularity point in space',result[0],result[4])
    ...
    for i in range(5):
        try:
            ...
            result = finalcondition(fq,1e6,xinit,yinit,pxinit,pyinit,0.0)
            if(digress == 0):
                ...
                if(real(result[4])>0.0 and abs(potential(result[0],result[2]))>1e3 and abs(result[0]-xtarget)<1e-2):# and real(result[4])<=4.0 and abs(imag(result[4]))<=2.0):
                    singtime_arr.append(result[4])
                    #print('diff',abs(result[0]-xtarget), 'for point',result[4])
    ...
    if(abs(initpointxcoord-xtarget)<1e-3 and real(initpointtime)>0.0):
        #print('here1',singtime_arr)
        return singtime_arr
    else:
        #print('here',singtime_arr)
        return singtime_arr[1:]
